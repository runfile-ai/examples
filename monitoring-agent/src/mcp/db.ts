// Shared pg pool for the MCP server. DSN points at the local `monitoring`
// database that holds both the ext.* and agent.* schemas.
import pg from "pg";

try { process.loadEnvFile(); } catch { /* env may come from the parent process */ }

const dsn = process.env.MONITORING_DB_DSN;
if (!dsn) {
  throw new Error("MONITORING_DB_DSN is not set");
}
