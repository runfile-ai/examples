"""End-to-end demo for the LangGraph build: one escalated, human-overridden decision.

Drives the SAME `mimic-creditline` MCP tools (no LLM / API key required) through
the full flow for the seeded escalation case, recording the LangGraph agent's
provenance, then simulates the officer override. Proves the environment +
provenance wiring without a model key.
...
# Reuse the environment layer from the sibling. Append (don't insert) so THIS
# project's `agent` package wins; only the sibling-only `mcp_servers` resolves
# from there.
...
async def main() -> None:
    print("=== Credit-Line Decision Agent (LangGraph) — deterministic demo ===\n")
