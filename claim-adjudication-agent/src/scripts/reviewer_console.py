"""Reviewer console — the human side of the decision gate. Lists pending approvals
(post_adjudication, and pended claims) and lets a reviewer approve or reject. The
agent's gated post_adjudication call is blocked until this resolves it.

  python -m src.scripts.reviewer_console            interactive
  python -m src.scripts.reviewer_console auto        unattended: approve the next pending one (demos)
"""
...
async def _resolve(conn: asyncpg.Connection, approval_id, status: str, decided_by: str) -> None:
    await conn.execute(
        "UPDATE agent.approvals SET status=$1, decided_by=$2, decided_at=now() WHERE id=$3", status, decided_by, approval_id
    )
...
async def auto() -> None:
    ...
    try:
        print("[reviewer:auto] waiting for a pending approval…", flush=True)
        while True:
            rows = await _pending(conn)
            if rows:
                a = rows[0]
                await _resolve(conn, a["id"], "approved", "reviewer-auto@demo")
                print(f"[reviewer:auto] APPROVED {a['action']} {a['payload']}", flush=True)
                return
...
async def interactive() -> None:
    ...
    try:
        ...
        for a in rows:
            ...
            if ans in ("approve", "a"):
                who = (await asyncio.to_thread(input, "  your reviewer id: ")).strip() or "reviewer-unknown"
