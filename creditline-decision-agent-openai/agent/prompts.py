"""Instructions and model/prompt provenance for the OpenAI Agents SDK runtime.
...
passes that exact value into ``creditline_record_decision`` so a recorded
decision pins precisely which prompt produced it — the same provenance contract
as the Claude version, computed locally here instead of via an MCP tool.
"""
...
AGENT_ID = "creditline-decision-agent-openai"
AGENT_VERSION = "0.1.0"
...
# Hash over the base instructions (before the provenance footer is appended),
# so the value is stable and self-consistent.
PROMPT_VERSION_HASH = "sha256:" + hashlib.sha256(BASE_INSTRUCTIONS.encode("utf-8")).hexdigest()

# What the agent actually receives: instructions + the exact provenance values
# to pass through to creditline_record_decision.
INSTRUCTIONS = (
    BASE_INSTRUCTIONS
    + "\n\nWhen recording the decision, pass these provenance values VERBATIM:\n"
    + f"  model_version       = {MODEL_VERSION}\n"
    + f"  prompt_version_hash = {PROMPT_VERSION_HASH}\n"
)
