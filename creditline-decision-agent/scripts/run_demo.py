"""End-to-end demo: one escalated, human-overridden credit-line decision.

Drives the REAL `mimic-creditline` MCP tools (no LLM / API key required) through
the full flow for the seeded escalation case, then simulates the credit officer
modifying the agent's recommendation — the Art. 14 / SR 11-7 effective-challenge
event. Useful as a deterministic smoke test of the environment + MCP server.
...
load_dotenv()
# Poll fast so the demo doesn't dawdle on the approval gate.
os.environ.setdefault("APPROVAL_POLL_SECONDS", "1")
os.environ.setdefault("APPROVAL_TIMEOUT_SECONDS", "60")

from agent.decision import evaluate  # noqa: E402
from agent.prompts import MODEL_VERSION, PROMPT_VERSION_HASH  # noqa: E402
from mcp_servers.mimic_creditline import server as S  # noqa: E402
from scripts.officer_console import _admin_dsn, resolve  # noqa: E402
...
async def main() -> None:
    print("=== Credit-Line Decision Agent — deterministic demo ===\n")
    ...
    print(f"Bureau score     : {bureau['credit_score']}  debt {bureau['total_outstanding_debt']}  "
          f"delinq {bureau['delinquencies_24m']}")
    ...
    # 4. Human-in-the-loop: agent blocks on the gate; officer resolves out of band.
    print("Opening human-in-the-loop approval gate (agent now blocks)…")
    summary = (
        f"{customer['full_name']} requests {req['requested_limit']}. "
        f"Above 15k ceiling and DTI {rec.dti:.3f} > 0.45. Recommend escalate; "
        f"suggest approving a reduced limit."
    )
    ...
    async def officer():
        ...
        await resolve(
            ...
            justification=(
                "Strong 7-yr relationship and clean delinquency record. Approve a "
                "reduced 12,000 limit to keep DTI within appetite; full 25,000 declined."
            ),
    ...
    # 5. Side effect
    note = await S.creditline_notify_customer(
        request_id=DEMO_REQUEST_ID,
        outcome="approved",
